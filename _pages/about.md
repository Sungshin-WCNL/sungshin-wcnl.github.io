---
permalink: /
title: "Wireless Communications and Network Lab @ Sungshin Women's University"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Welcome to the **Wireless Communications and Network Laboratory (WCNL)** at Sungshin Women's University.

Our research focuses on the design, analysis, and optimization of **Next-Generation Wireless Communication Systems**. We are dedicated to developing core technologies for future hyper-connected societies, including 6G mobile communications, AI-native wireless networks, and ultra-reliable low-latency communications.

## Research Interests
Our lab explores a wide range of topics in wireless connectivity and intelligent networking:

* **Stochastic Geometry-based Network Analysis**: Modeling and analyzing large-scale wireless networks (e.g., Poisson Point Processes) to derive key performance metrics like coverage probability and spectral efficiency.
* **Next-Generation Mobile Communications (6G & Beyond)**: Architecture design and protocol optimization for future cellular networks using spatial modeling.
* **AI-Driven Wireless Networks**: Integrating machine learning with stochastic models for intelligent radio resource management and network optimization.

## News
* **[Apr. 2026]** We are looking for motivated undergraduate and graduate students. If you are interested in joining WCNL, please contact us.
* **[Apr. 2026]** The WCNL lab website is officially launched!


---

## Contact
* **Email**: [junselee@sungshin.ac.kr](mailto:junselee@sungshin.ac.kr)
* **Office**: A-709, Sujeong-Gwan, Sungshin Women's University (Sujeong Campus), Seoul, Korea